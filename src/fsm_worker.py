...
class FSMWorker:
    ...
    def _read_zone_snapshot(self, zone_name: str, device_config: Dict[str, Any]) -> Optional[ZoneSnapshot]:
        ...
        try:
            ...
            # Get setpoint from database (populated by Modbus poller)
            setpoint_data = self.db_manager.get_setpoint(zone_name)
            if setpoint_data and setpoint_data.get('setpoint_value') is not None:
                setpoint = setpoint_data['setpoint_value']
                logger.debug(f"Read setpoint for {zone_name} from database: {setpoint}°F")
            else:
                # Fallback to default if no setpoint data available
                setpoint = 150.0
                logger.debug(f"No setpoint data for {zone_name}, using default: {setpoint}°F")
